...
type DrgRouteDistributionStatement struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.drgRouteDistributionId) || (has(self.initProvider) && has(self.initProvider.drgRouteDistributionId))",message="spec.forProvider.drgRouteDistributionId is a required parameter"
