...
type IPv6 struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.vnicId) || (has(self.initProvider) && has(self.initProvider.vnicId))",message="spec.forProvider.vnicId is a required parameter"
