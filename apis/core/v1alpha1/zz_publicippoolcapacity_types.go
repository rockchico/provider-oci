...
type PublicIPPoolCapacity struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.publicIpPoolId) || (has(self.initProvider) && has(self.initProvider.publicIpPoolId))",message="spec.forProvider.publicIpPoolId is a required parameter"
