...
type StorageExportSpec struct {
	...
	// autoscaler.
	InitProvider StorageExportInitParameters `json:"initProvider,omitempty"`
}
...
type StorageExport struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.exportSetId) || (has(self.initProvider) && has(self.initProvider.exportSetId))",message="spec.forProvider.exportSetId is a required parameter"
