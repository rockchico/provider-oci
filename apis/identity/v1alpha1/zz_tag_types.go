...
type Tag struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.tagNamespaceId) || (has(self.initProvider) && has(self.initProvider.tagNamespaceId))",message="spec.forProvider.tagNamespaceId is a required parameter"
